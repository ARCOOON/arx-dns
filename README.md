...
| `server.root_hints_file`         | `./named.root`                                | Path to the IANA root hints zone file used for iterative resolution                                                                                                                    |
| `server.auto_update_root_hints`  | `true`                                        | When `true`, download or refresh the root hints file from InterNIC when missing or older than 30 days; when `false`, read only the local file (for Ansible/Puppet-managed deployments) |

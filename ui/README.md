...
| `/zones`      | Zones & Records | Zone sidebar with **Add Zone**, record table (name + muted FQDN), reactive **Add/Edit Record** dialog (type-specific fields for MX/SRV), **AlertDialog** confirmation for zone and record delete, record update via `PUT /api/v1/zones/{zone}/records/{id}`                                                                                                 |

...
- [x] **Zone & Record Management API (Phase 17, Phase 46):** Authenticated `GET /api/v1/zones`, `GET /api/v1/zones/{zone}/records`, `POST /api/v1/zones/{zone}/records`, `DELETE /api/v1/zones/{zone}/records`, and `DELETE /api/v1/zones/{zone}/records/{id}`; atomic radix-tree swap on mutation; BIND `.zone` file rewrite via `internal/storage` writer with atomic rename; stable SHA-256 record IDs for path-based delete.

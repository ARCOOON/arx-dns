...
- [x] **Unified TOML Configuration:** Single `config.toml` file with embedded-template auto-generation on first start; automatic creation of `./data`, `./data/certs`, and `./zones` on boot; `[tls]` and `[listeners]` sections for encrypted DNS; `[security]` section for DNSSEC validation and DNS Cookies; `[ecs]` section for EDNS Client Subnet forwarding; `[resolver]` section for forward vs iterative recursive mode; autonomous root hints via `server.root_hints_file` and `server.auto_update_root_hints` (legacy `resolver.root_hints` removed); `[xfr]` section for zone transfer ACLs and NOTIFY slaves; all legacy CLI flags migrated to typed `internal/config` struct (Phase 12, Phase 14, Phase 16, Phase 22, Phase 23, Phase 28, Phase 29, Phase 41).
...
- [x] **Automated bootstrapping & config audit (Phase 41):** `config.Bootstrap()` in `cmd/arx-dns/main.go` creates `./data`, `./data/certs`, and `./zones` on startup; writes embedded default TOML when config is missing (`server.root_hints_file`, `server.auto_update_root_hints`); removed deprecated `resolver.root_hints` field, defaults, validation, and `NormalizedRootHints()`.
